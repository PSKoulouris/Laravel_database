...
So when someone visits /updateStudentRoute with a POST request, Laravel calls: StudentController::updateStudent()

4. ->name("updateStudent")

This gives the route a name (an alias).
Instead of hardcoding /updateStudentRoute in your Blade files, you can use: <form method="post" action="{{ route('updateStudent', $itemStudent->id) }}">

This way:

If you ever change the URL (e.g., /students/update instead of /updateStudentRoute), you only update it in the route file, not in every form.
*/
